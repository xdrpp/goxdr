// Code generated by goxdr -enum-comments -p rpc -o rpc/rpc_msg.go rpc/rpc_msg.x; DO NOT EDIT.
...
var _XdrComments_Accept_stat = map[int32]string {
	int32(SUCCESS): "RPC executed successfully",
	int32(PROG_UNAVAIL): "remote hasn't exported program",
	int32(PROG_MISMATCH): "remote can't support version #",
	int32(PROC_UNAVAIL): "program can't support procedure",
	int32(GARBAGE_ARGS): "procedure can't decode params",
	int32(SYSTEM_ERR): "e.g. memory allocation failure",
}
func (e Accept_stat) XdrEnumComments() map[int32]string {
	return _XdrComments_Accept_stat
}
...
var _XdrComments_Reject_stat = map[int32]string {
	int32(RPC_MISMATCH): "RPC version number != 2",
	int32(AUTH_ERROR): "remote can't authenticate caller",
}
func (e Reject_stat) XdrEnumComments() map[int32]string {
	return _XdrComments_Reject_stat
}
...
var _XdrComments_Auth_stat = map[int32]string {
	int32(AUTH_OK): "success",
	int32(AUTH_BADCRED): "bad credential (seal broken)",
	int32(AUTH_REJECTEDCRED): "client must begin new session",
	int32(AUTH_BADVERF): "bad verifier (seal broken)",
	int32(AUTH_REJECTEDVERF): "verifier expired or replayed",
	int32(AUTH_TOOWEAK): "rejected for security reasons",
	int32(AUTH_INVALIDRESP): "bogus response verifier",
	int32(AUTH_FAILED): "reason unknown",
	int32(AUTH_KERB_GENERIC): "kerberos generic error",
	int32(AUTH_TIMEEXPIRE): "time of credential expired",
	int32(AUTH_TKT_FILE): "problem with ticket file",
	int32(AUTH_DECODE): "can't decode authenticator",
	int32(AUTH_NET_ADDR): "wrong net address in ticket",
	int32(RPCSEC_GSS_CREDPROBLEM): "no credentials for user",
	int32(RPCSEC_GSS_CTXPROBLEM): "problem with context",
}
func (e Auth_stat) XdrEnumComments() map[int32]string {
	return _XdrComments_Auth_stat
}
...
func (_ XdrAnon_Accepted_reply_Reply_data) XdrValidTags() map[int32]bool {
	return nil
}
func (u *XdrAnon_Accepted_reply_Reply_data) Results() *[0]byte {
	switch u.Stat {
	case SUCCESS:
		if v, ok := u._u.(*[0]byte); ok {
			return v
		} else {
			var zero [0]byte
			u._u = &zero
...
func (u *XdrAnon_Accepted_reply_Reply_data) Mismatch_info() *XdrAnon_Accepted_reply_Reply_data_Mismatch_info {
	switch u.Stat {
	case PROG_MISMATCH:
		if v, ok := u._u.(*XdrAnon_Accepted_reply_Reply_data_Mismatch_info); ok {
			return v
		} else {
...
func (u *Rejected_reply) Mismatch_info() *XdrAnon_Rejected_reply_Mismatch_info {
	switch u.Stat {
	case RPC_MISMATCH:
		if v, ok := u._u.(*XdrAnon_Rejected_reply_Mismatch_info); ok {
			return v
		} else {
...
func (u *Reply_body) Areply() *Accepted_reply {
	switch u.Stat {
	case MSG_ACCEPTED:
		if v, ok := u._u.(*Accepted_reply); ok {
			return v
		} else {
...
func (u *Reply_body) Rreply() *Rejected_reply {
	switch u.Stat {
	case MSG_DENIED:
		if v, ok := u._u.(*Rejected_reply); ok {
			return v
		} else {
